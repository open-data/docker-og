...
If using inside of WSL2, the nginx and postgres containers map to `0.0.0.0` so the containers should be discoverable through your network. You will just need to add your WSL2 ip address to the Windows hosts file.

1. Get your WSL2 ip address: `wsl hostname -I`;
1. Edit `C:\Windows\System32\drivers\etc\hosts` as an admin;
1. Add the following:
    ```
    # Hosts generated by the OG CLI for use with Docker
    <wsl2 ip address>	open.local
    <wsl2 ip address>	blog.open.local
    <wsl2 ip address>	guides.open.local
    <wsl2 ip address>	registry.open.local
    <wsl2 ip address>	portal.open.local
    <wsl2 ip address>	solr.open.local
    <wsl2 ip address>	search.open.local
    # End of OG CLI hosts
    ```
